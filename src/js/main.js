...
(function ($) {
   let sportShop = {
      ...
      // mobile menu
      mobileMenu: function() {
         let mobileMenu = $('.mobile-nav-menu');
         let self = this;
         let expandIcon = '<span class="fas fa-plus expand-btn"></span>';
         let mobileMenuBtn = $('.mobile-menu-btn');
         let categoriesMenu = $('.categories');
         //let categoryExpandBtn = $('.categories .expand-btn, .categories .column > a i');
         ...
         this._window.on('resize load', function() {
            if (self.windowSize <= 991) {
               ...
               $('.nav-menu > ul')
                  .clone()
                  .appendTo(mobileMenu)
                  .find('.column > ul > li').addClass('drop-menu').end()
                  .find('.drop-menu').parent().children('a').append(expandIcon);
               ...
               categoriesMenu.addClass('mobile')
                  .find('._dropdown-menu > li > a').append('<i class="fas fa-angle-down expand-btn"></i>');

               // categories menu expand button
               ...
               $('.categories .expand-btn, .categories .column > a i').off('click');
               // $('.category-menu').css('display', 'block');
               $('.category-menu ._dropdown-menu').removeAttr('style')
                  .find('li > ul').removeAttr('style');
               mobileMenuBtn.off('click');
               mobileMenu.children().remove();
               categoriesMenu.removeClass('mobile')
                  .find('._dropdown-menu > li > a i').remove();
            }
         });
      ...
      sliderStyle_1: () => {
         $('.slider-style1').each(function() {
            let $arrowContainer = $(this).prev().find('.slider-arrows');
            let $slidesToShow = $(this).attr('data-show') ? parseInt($(this).attr('data-show')) : 1;
            $(this).slick({
               swipeToSlide: true,
      ...
      sliderStyle_2: () => {
         $('.slider-style2').each(function() {
            let arrowsContainer = $(this).parent().find('.slider-arrows2');
            $(this).slick({
               swipeToSlide: true,
               arrows: false,
      ...
      sliderStyle_3: () => {
         $('.slider-style3').each(function() {
            let $arrowsContainer = $(this).parent().find('.slider-arrows');
            $(this).slick({
               swipeToSlide: true,
      ...
      quantityChange: () => {
         $('.js-quantity').on('click', '.c-quantity_btn', function() {
            let $qty_input = $(this).parent().find('.c-quantity_value');
            let $value = parseInt($qty_input.val(), 10);
      ...
      paymentMethodCollapse: () => {
         $('input[name=paymentMethod]').on('click', function() {
            let parent = $(this).closest('[data-parent="collapse-parent"]');
